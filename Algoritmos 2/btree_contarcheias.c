...
void contar_paginas(NodeB *tree, int *contador){
    int i;
    if(tree != NULL){
        *contador = *contador + 1;
        ...
        if(tree->filhos[0] != NULL){
            ...
            else{//caso a pagina filha n seja folha
                for(i=0;i<tree->nro_chaves;i++){
                    contar_paginas(tree->filhos[i],contador);
                }
            }
        }
    }
...
void contar_paginascheias(NodeB *tree, int *contador){
    int i;
    if(tree != NULL){
        *contador = *contador + (tree->nro_chaves == N - 1);
        ...
        if(tree->filhos[0] != NULL){
            ...
            for(i=0;i<tree->nro_chaves;i++){
                contar_paginascheias(tree->filhos[i],contador);
            }
        }
    }
...
int main()
{
   NodeB *tree = criar();
    ...
    contar_paginascheias(tree,&contador);
    printf("%d", contador);
}
