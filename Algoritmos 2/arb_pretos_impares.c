...
int contar_pretos_impares(NodeVP* tree){
    if(tree!=NULL){
        if((tree->color == 1) && (tree->key%2 != 0)){
            return contar_pretos_impares(tree->right) + contar_pretos_impares(tree->left) + 1;
        }
        else{
            return contar_pretos_impares(tree->right) + contar_pretos_impares(tree->left);
        }
    }
    return 0;
}
int main(){
    ...
    printf("%d",contar_pretos_impares(tree));
}
