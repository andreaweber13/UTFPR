...
int somar_folhas(NodeB* tree){
    ...
    if(tree != NULL){
        if(tree->eh_no_folha){
            ...
            for(i=0;i<=tree->nro_chaves;i++){
                soma += somar_folhas(tree->filhos[i]);
            }
        }
    }
    return soma;
}
int contar_folhas(NodeB* tree){
    int i, qtde=0;
    if(tree != NULL){
        if(tree->eh_no_folha){
            for(i=0;i<tree->nro_chaves;i++){
            ...
            for(i=0;i<=tree->nro_chaves;i++){
                qtde += contar_folhas(tree->filhos[i]);
            }
        }
    }
    return qtde;
...
int main()
{
    ...
    printf("%d", (somar_folhas(tree))/(contar_folhas(tree)));
